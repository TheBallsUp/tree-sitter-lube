...
bool tree_sitter_lube_external_scanner_scan(void *payload, TSLexer *lexer,
            ...
            if (lexer->lookahead == '\\') {
                ...
                if (is_bytes(&delimiter)) {
                    ...
                    if (lexer->lookahead == 'N' || lexer->lookahead == 'u' ||
                        ...
                        // https://docs.lube.org/3/reference/lexical_analysis.html#string-and-bytes-literals
                        advance(lexer);
                    } else {
                        lexer->result_symbol = STRING_CONTENT;
                        return has_content;
...
unsigned tree_sitter_lube_external_scanner_serialize(void *payload,
                                                       char *buffer) {
    Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_lube_external_scanner_deserialize(void *payload,
                                                     const char *buffer,
                                                     unsigned length) {
    Scanner *scanner = (Scanner *)payload;
